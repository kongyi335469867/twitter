...
<script type="text/javascript">
	...
	function siliao() {
	 	var url =  '/twitter/message.do?method=hasnew';
		$.ajax({
			url : url,
			type : 'POST',
	...
	function shishitongzhi() {
		$.ajax({
			url : '/twitter/notify.do?method=noread',
			type : 'POST',
			success : function(response, status) {
...
<script>
	...
	function xiugaishengri(data) {
		$.ajax({
			url : '/twitter/user.do?method=updatebrithday',
			type : 'POST',
			data : {
				date : data   //生日:数据
			},
	...
	function gengxininfo(data) {
		$.ajax({
			url : '/twitter/user.do?method=gengxininfo&neirong='+data,
			type : 'POST',
			success : function(response, status) {
				if(response == 'ok'){
	...
	function xiugaijianjie(data) {
		$.ajax({
			url : '/twitter/user.do?method=updateabout',
			type : 'POST',
			data : {
				about : data
			},
...
<script>
	...
	function hasNew() {
		$.ajax({
			url : '/twitter/tweettwo.do?method=gettweets&pagenum=0',
			type : 'POST',
			success : function(response, status) {
	...
	function jiaZai() {
		if( $("#back").css("display") == "none"){
			$('#jiazai img').fadeIn();
			var url = '/twitter/tweettwo.do?method=gettweets&pagenum='+$(".tuiwen").length;
			$.ajax({
				url : url,
				type : 'POST',
				success : function(response, status) {
	...
	function gengXin() {
	 	var url =  '/twitter/tweettwo.do?method=getnewtweets&td=top&nowid='+$(".tuiwen:eq(0)").find(".idid").val();
		$.ajax({
			url : url,
			type : 'POST',
			success : function(response, status) {
	...
	function tianJia() {
	 	var url =  '/twitter/tweettwo.do?method=getnewtweets&td=down&nowid='+$(".tuiwen:eq(0)").find(".idid").val();
		$.ajax({
			url : url,
			type : 'POST',
			success : function(response, status) {
	...
	for (var i = 0; i < tweets.length; i++) {
				 if(tweets[i].tcontent =="" && tweets[i].tpic == null && tweets[i].tzhaun == 0){
					...
					+ twitter+'/img/'+tweets[i].uname+'/'+tweets[i].ulogo+ '\');background-size: 60px 60px;"></div><div class="sender_info"><div onmouseover="nameTianZiLiao(this)" onmouseout="removeZiLiaoKatwo(this)" class="sender_name" style="z-index:99;font-weight: bold;">'
	...
	function getreply(tid,page,aite){
			$.ajax({
			url : '/twitter/reply.do?tid='+tid+'&page='+page,
			type : 'POST',
			success : function(response, status) {
	...
	function addReply(replys,aite){
		...
		for(var i = 0; i < replys.length; i++){
			...
			+ twitter+'/img/'+replys[i].uname+'/'+replys[i].ulogo+'" /></div><div><div class="huifudename">'
...
function tweetsJs(){
	...
	$("#submit").click(function(){
		if($(".meiyou").css("display") == "block"){
			var url =  '/twitter/tweettwo.do?method=getnewtweets&td=down&nowid=';
		}else{
			var url =  '/twitter/tweettwo.do?method=getnewtweets&td=down&nowid='+$(".tuiwen:eq(0)").find(".idid").val();
		}
		$.ajax({
...
<script>
			...
			$.ajax({
				...
				success : function(data) {
					if (data == "ok") {
						...
						+'<div style="width: 60px;height: 100%;float:left;"><img style="width:48px;height:48px;border-radius: 50px" src="'
						+ twitter+'/img/'+"${user.uname}"+'/'+"${info.ulogo}"+'" /></div><div><div class="huifudename">'
	...
	function addZiLiaoKa(tweets, state) {
		...
		for (var i = 0; i < tweets.length; i++) {
			...
			if (tweets[i].ubg == null) {
				...
				html += 'style="background-color: ' + tweets[i].ucolor + ';background: url(' + twitter + '/img/' + tweets[i].uname + "/" + tweets[i].ubg + ');';
			}
			html += 'background-size: 300px 100px;"></div><div style="width:300px;height:200px;"><div class="waiyuan">' +
				'<div class="yuantouxiang" style="background: url(' + twitter + '/img/' + tweets[i].uname + "/" + tweets[i].ulogo + ');background-size: 70px 70px;"></div></div>'
	...
	$(function() {
		...
		$('#btnCrop').on('click', function() {
			...
			var url = '/twitter/user.do?method=xiugaitouxiang';
			$.ajax({
				url : url,
				data : {
					"touxiang" : img
